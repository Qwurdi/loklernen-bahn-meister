...
export default function BottomNavigation() {
  ...
  return (
    <nav className="md:hidden fixed bottom-0 left-0 right-0 flex justify-around items-center bg-black border-t border-gray-800 z-50 h-18 py-2">
      {navItems.map((item) => (
        <Link
          key={item.path}
          to={item.path}
        ...
        >
          <div className={cn(
            ...
            isActive(item.path) ? "bg-blue-900/30" : ""
          )}>
            {item.icon}
          </div>
          <span>{item.name}</span>
